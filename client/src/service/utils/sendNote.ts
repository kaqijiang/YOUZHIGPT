...
const emailMap: { [key: string]: any } = {
  [UserAuthTypeEnum.register]: {
    subject: '注册 YOUZHI 账号',
    // html: (code: string) => `<div>您正在注册 YOUZHI 账号，验证码为：${code}</div> <div> 更多相关内容：https://bento.me/aijie </div> `

    html: (code: string) => `
        <div>
            <p>您正在注册 YOUZHI 账号，验证码为：<strong>${code}</strong></p>
            <p>更多相关内容请访问：<a href="https://bento.me/aijie">https://bento.me/aijie</a></p>
            <p>感谢您的支持！</p>
        </div>
      `
  },
  [UserAuthTypeEnum.findPassword]: {
    subject: '修改 YOUZHI 密码',
    html: (code: string) => `
        <div>
            <p>您正在修改 YOUZHI 账号密码，验证码为：<strong>${code}</strong></p>
            <p>更多相关内容请访问：<a href="https://bento.me/aijie">https://bento.me/aijie</a></p>
            <p>感谢您的支持！</p>
        </div>
    `
